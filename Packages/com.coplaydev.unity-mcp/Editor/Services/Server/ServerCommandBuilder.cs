...
namespace MCPForUnity.Editor.Services.Server
{
    ...
    public class ServerCommandBuilder : IServerCommandBuilder
    {
        ...
        public bool TryBuildCommand(out string fileName, out string arguments, out string displayCommand, out string error)
        {
            ...
            // GetUvxPath falls back to uv when the uvx alias is unavailable. The
            // following flags belong to `uv tool run`, rather than uv's root
            // command, so add the subcommand for that fallback case.
            bool isUvFallback = string.Equals(
                Path.GetFileNameWithoutExtension(uvxPath),
                "uv",
                StringComparison.OrdinalIgnoreCase
            );
            string toolRunPrefix = isUvFallback ? "tool run " : string.Empty;

            string args = string.IsNullOrEmpty(fromArgs)
                ? $"{toolRunPrefix}{devFlags}{packageName} --transport http --http-url {httpUrl}{scopedFlag}"
                : $"{toolRunPrefix}{devFlags}{fromArgs} {packageName} --transport http --http-url {httpUrl}{scopedFlag}";
