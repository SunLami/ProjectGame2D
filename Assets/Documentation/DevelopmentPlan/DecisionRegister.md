...
| D-034 | Compact scrollable QuestTracker | **Accepted từ yêu cầu — 2026-09-08** | Thay tracker parchment bằng viewport HUD cố định `190×230`, nền trong suốt và chỉ xám mờ khi hover. Quest Active/ReadyToTurnIn xếp Main → Side → Daily, cuộn bằng mouse wheel, mỗi title bung/thu objective ngắn độc lập; QuestLogWindow giữ nội dung đầy đủ. Daily category chỉ là presentation metadata, không quyết định reset clock/generation. Ba nút dưới `DEBUG LEVEL +1` nhận quest `quest.debug.*` chỉ trong Editor/Development Build; các quest này không persist và không làm session dirty. | Content production |
