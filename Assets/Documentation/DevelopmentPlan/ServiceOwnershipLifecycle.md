...
| `MusicManager` | Static singleton, `DontDestroyOnLoad`; volume do SettingsService áp | AudioSource cùng root; không giữ scene object ngoài root | **Application audio service** | API static; clip chưa được cấu hình | Phase 1 settings ownership đã hoàn tất; giữ implementation nhỏ cho tới khi có music content |
